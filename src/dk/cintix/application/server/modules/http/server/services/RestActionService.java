...
import dk.cintix.application.server.modules.http.server.services.domain.models.CachedResponse;
...
public class RestActionService {
    ...
    public RestActionService(RestEndpoint endpoint, List<String> arguments) {
    ...
    public Response process(RestHttpRequest request) {
        try {
            ...
            dk.cintix.application.server.infrastructure.Cache cache = _CACHE_MAPS.get(cacheBaseId);
            ...
            if (useCache) {
                if (cache == null) {
                    ...
                    if (cacheByStatus == null) {
                        cache = new dk.cintix.application.server.infrastructure.Cache<String, String>(1);
                    } else {
                        ...
                        for (Cache cacheOptions : cacheByStatus.value()) {
                            if (isStatusDefinedInCache(cacheOptions.status(), currentStatusCode)) {
                                cache = new dk.cintix.application.server.infrastructure.Cache<String, String>(cacheOptions.timeToLive(), cacheOptions.size());
